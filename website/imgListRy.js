...
function getItemList(){
	//var param = "SITEM_ID=" + $("#ITEM_ID").val() + "&page="+ (curPage+1) +"&rows=" + pageSize + "&ORDERTITLE=1";
...
function appendItemList(data){
	...
	if(totalCount >0){
		for(var i=0,len=data.itemList.length;i<len;i++){
			...
			htmlStr.push('<div class="fl pics2" style="margin-right:16px;">');
			htmlStr.push('<div class="pic">');
			htmlStr.push('<div class="img">');
			htmlStr.push('<a href="' + baseUrl +"/pub/news/"+ item.ID + '" title="' + item.TITLE + '">');
			htmlStr.push('<img src="' + item.FMTPURL + '" width="172" height="230" border="0">');
			htmlStr.push('</a>');
			htmlStr.push('</div>');
			htmlStr.push('</div>');
